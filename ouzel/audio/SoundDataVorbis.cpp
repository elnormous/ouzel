...
namespace ouzel
{
    namespace audio
    {
        ...
        bool SoundDataVorbis::getData(Stream* stream, uint32_t frames, bool repeat, bool& finished, std::vector<float>& result)
        {
            ...
            while (neededSize > 0)
            {
                if (vorbisStream->eof) stb_vorbis_seek_start(vorbisStream);
