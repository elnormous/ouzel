        ...
        void D3D11Texture::createTexture(const std::vector<Texture::Level>& levels)
        {
            ...
            if (flags & Texture::RENDER_TARGET)
            {
                ...
                if (sampleCount > 1)
                {
                    ...
                    if (FAILED(hr = renderDeviceD3D11.getDevice()->CreateTexture2D(&msaaTextureDescriptor, nullptr, &msaaTexture)))
                        throw std::system_error(hr, direct3D11ErrorCategory, "Failed to create Direct3D 11 texture");
                    
                    if (FAILED(hr = renderDeviceD3D11.getDevice()->CreateRenderTargetView(msaaTexture, &renderTargetViewDesc, &renderTargetView)))
                        throw std::system_error(hr, direct3D11ErrorCategory, "Failed to create Direct3D 11 render target view");
                }
                else if (FAILED(hr = renderDeviceD3D11.getDevice()->CreateRenderTargetView(texture, &renderTargetViewDesc, &renderTargetView)))
                    throw std::system_error(hr, direct3D11ErrorCategory, "Failed to create Direct3D 11 render target view");
                ...
                if (flags & Texture::DEPTH_BUFFER)
                {
                    ...
                    if (FAILED(hr = renderDeviceD3D11.getDevice()->CreateTexture2D(&depthStencilDescriptor, nullptr, &depthStencilTexture)))
