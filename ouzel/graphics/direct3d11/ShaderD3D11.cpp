        ...
        bool ShaderD3D11::upload()
        {
            if (uploadData.dirty)
            {
                ...
                if (!pixelShader)
                {
                    ...
                    if (FAILED(hr))
                    {
                        log(LOG_LEVEL_ERROR, "Failed to create a Direct3D 11 pixel shader");
                        return false;
                    }
                }
                ...
                if (!vertexShader)
                {
                    ...
                    if (FAILED(hr))
                    {
                        log(LOG_LEVEL_ERROR, "Failed to create a Direct3D 11 vertex shader");
                        return false;
                    }
                    ...
                    if (FAILED(hr))
                    {
                        log(LOG_LEVEL_ERROR, "Failed to create Direct3D 11 input layout for vertex shader");
                        return false;
                    }
                }
                ...
                if (FAILED(hr))
                {
                    log(LOG_LEVEL_ERROR, "Failed to create Direct3D 11 constant buffer");
                    return false;
                }
