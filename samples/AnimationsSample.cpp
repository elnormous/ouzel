    ...
    backButton("button.png", "button_selected.png", "button_down.png", "", "Back", "arial.fnt", 1.0F, Color::BLACK, Color::BLACK, Color::BLACK)
{
    ...
    shapeDrawable.line(Vector2<float>(0.0F, 0.0F), Vector2<float>(50.0F, 50.0F), Color::CYAN, 2.0F);

    shapeDrawable.curve({Vector2<float>(50.0F, 50.0F),
                         Vector2<float>(100.0F, 50.0F),
                         Vector2<float>(50.0F, 0.0F),
                         Vector2<float>(100.0F, 0.0F)},
                        Color::YELLOW);

    shapeDrawable.circle(Vector2<float>(25.0F, 75.0F), 20.0F, Color::BLUE, true);
    shapeDrawable.circle(Vector2<float>(25.0F, 75.0F), 20.0F, Color::WHITE, false);
    shapeDrawable.circle(Vector2<float>(75.0F, 75.0F), 20.0F, Color::BLUE, false, 16, 4.0F);

    shapeDrawable.polygon({Vector2<float>(15.0F, 75.0F),
                           Vector2<float>(25.0F, 75.0F),
                           Vector2<float>(25.0F, 55.0F)},
                          Color("#ff0000"), false);

    drawActor.addComponent(&shapeDrawable);
    drawActor.setPosition(Vector2<float>(-300, 0.0F));
    layer.addChild(&drawActor);

    shake.reset(new scene::Shake(10.0F, Vector3<float>(10.0F, 20.0F, 0.0F), 20.0F));
    drawActor.addComponent(shake.get());
    shake->start();

    witchSprite.init("witch.png");

    witch.setPosition(Vector2<float>(200, 0.0F));
    witch.addComponent(&witchSprite);
    layer.addChild(&witch);

    witchScale.reset(new scene::Scale(2.0F, Vector3<float>(0.1F, 0.1F, 0.0F), false));
    ...
    witchRotate.reset(new scene::Rotate(1.0F, Vector3<float>(0.0F, 0.0F, TAU), false));
    ...
    ballMove.reset(new scene::Move(2.0F, Vector3<float>(0.0F, -240.0F, 0.0F), false));
