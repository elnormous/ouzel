            ...
            apiVersion{version}
        {
            ...
            if (apiVersion >= ApiVersion{3, 0})
            {
                ...
                if (const auto error = glGetErrorProc(); error != GL_NO_ERROR)
                    log(Log::Level::warning) << "Failed to get OpenGL extension count, error: " + std::to_string(error);
                else
                    for (GLuint i = 0; i < static_cast<GLuint>(extensionCount); ++i)
                    {
                        ...
                        if (const auto getStringError = glGetErrorProc(); getStringError != GL_NO_ERROR)
                            log(Log::Level::warning) << "Failed to get OpenGL extension, error: " + std::to_string(getStringError);
                        else if (!extensionPtr)
                            log(Log::Level::warning) << "Failed to get OpenGL extension";
                        else
            ...
            else
            {
                ...
                if (const auto error = glGetErrorProc(); error != GL_NO_ERROR)
                    log(Log::Level::warning) << "Failed to get OpenGL extensions, error: " + std::to_string(error);
